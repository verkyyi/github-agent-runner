...
4. Human override at any time: add `state:blocked` to halt, edit a comment to steer the next agent, or manually re-dispatch a specific role to retry a stuck stage. Manual dispatches **must** pass all required inputs — the agents fail loudly if inputs are missing rather than inferring from labels.

   ```bash
   # Re-dispatch a specific stage manually (use exact field names):
   ...
   gh workflow run implementer-agent -f issue_number=<N> -f iteration=<K> -f pr_number=<P>  # kickback path
   gh workflow run reviewer-agent    -f pr_number=<P>    -f issue_number=<N> -f iteration=<K>
   ```

5. **Retrying a blocked task**:
   - *Normal block* (`state:blocked` added by a human or by an agent that hit an ambiguity): clear `state:blocked`, then re-add `agent-team`. Spec-agent treats it as a fresh dispatch. To redo from scratch, also delete the prior spec comment.
   - *Missing-inputs block* (agent posted `🛑 agent-team: workflow_dispatch inputs were not propagated`): the automated dispatch chain broke mid-flight. Re-dispatch the affected stage directly via `gh workflow run` with explicit inputs as shown above — do **not** re-add `agent-team`, which would restart from spec.
...
- **Input propagation**: planner / implementer / reviewer fail loudly if required `workflow_dispatch` inputs are missing or unresolved — they post `🛑 agent-team: workflow_dispatch inputs were not propagated` and add `state:blocked`. Recovery: re-dispatch the blocked stage via `gh workflow run` with explicit inputs (see step 4 above). Do **not** re-add `agent-team` — that restarts from spec.
