...
output=$(run_claude "What is the install-workflow skill? Describe its key steps briefly." 45)
...
output=$(run_claude "What auth options does install-workflow support? Describe the OAuth path and the API-key path." 45)
...
output=$(run_claude "In the install-workflow skill, why must the line '--exclude-env ANTHROPIC_API_KEY' be preserved rather than replaced with CLAUDE_CODE_OAUTH_TOKEN?" 60)
...
output=$(run_claude "Does install-workflow ever write workflow YAML by hand, or does it always delegate? Answer directly." 45)
...
output=$(run_claude "Does install-workflow store, echo, or see the user's auth token directly? Answer directly." 45)
