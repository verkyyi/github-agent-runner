...
output=$(run_claude "What is the install-agent-team skill? Describe what it installs." 45)
...
output=$(run_claude "After installing the agent-team via install-agent-team, what does a user do to dispatch a task to the agent pipeline?" 45)
...
output=$(run_claude "If one of the four workflow installs fails during install-agent-team, what does the skill do?" 60)
...
output=$(run_claude "Does install-agent-team ask for auth separately for each of the four workflows, and does it apply the OAuth token tweak to one or to all of the generated .lock.yml files?" 60)
...
output=$(run_claude "Does install-agent-team create GitHub labels as part of installation, or must the user create them manually? List the labels it creates." 60)
...
output=$(run_claude "In the install-agent-team skill, does Claude ever write workflow YAML directly, or store/echo the user's auth token? Answer directly." 45)
