...
output=$(run_claude "Load the install-agent-team skill. Per its Hard rules, if one of the four 'gh aw add' calls fails mid-install, what does the skill do — continue, skip, or abort? And why (what does the skill say about a half-installed pipeline)? Quote the exact hard-rule text." 180)
...
output=$(run_claude "Load the install-agent-team skill. Does it configure the Claude auth secret once (reused across all four workflows) or set it separately per workflow? And does it apply the OAuth post-compile tweak to one .lock.yml file or to every generated .lock.yml? Quote the specific steps from the skill." 180)
